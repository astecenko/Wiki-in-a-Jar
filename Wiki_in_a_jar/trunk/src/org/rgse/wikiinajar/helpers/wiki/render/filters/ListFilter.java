...
public class ListFilter implements Filter {
	...
	private static String generateList(String text) {
		...
		for (int i = 0; i < lines.length; i++) {
			...
			if (lineLevel > 0) {
				...
				String[] listData = extractListData(line.substring(level)
						.trim());
				buffer.append("\t<li " + listData[0] + " >" + listData[1]
						+ "</li>\n");
			}
		}
	...
	/**
	 * Checks if the list item starts with a to do marker
	 * 
	 * @param string
	 * @return the list item id (for styling) as first element, the list item
	 *         line as second.
	 */
	private static String[] extractListData(String string) {
